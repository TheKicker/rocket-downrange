      ...
      <div class="card-body p-3 p-md-6 m-md-3">
        ...
        <div class="row container mx-auto">
          <div class="my-4">
            ...
            >This picture was taken on Earth date: {{new Date(results.photos[0].earth_date).toDateString()}}, or Sol {{results.photos[0].sol}} on Mars. Captured with the {{results.photos[0].camera.full_name}} ({{results.photos[0].camera.name}}) aboard the {{results.photos[0].rover.name}} rover. {{results.photos[0].rover.name}} was launched from Earth on {{new Date(results.photos[0].rover.launch_date).toDateString()}} and has been on Mars since {{new Date(results.photos[0].rover.landing_date).toDateString()}}. Sadly, we lost communication with {{results.photos[0].rover.name}} in June of 2018 and mission was declared {{results.photos[0].rover.status}} on February 13th, 2019. All images and mission data compliments of NASA. RIP Oppy.</p>
          </div>
        </div>
        <hr />
      </div>
